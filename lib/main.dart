...
class _InvincibleOverlayState extends State<InvincibleOverlay> {
  ...
  Color _flashBgColor = Colors.transparent; 
  ...
  void initState() {
    ...
    FlutterOverlayWindow.overlayListener.listen((event) async {
      ...
      try {
        if (!_distFocus.hasFocus && !_isShrunk && !_isGhostMode) {
  ...
  void _triggerFlash(Color flashColor) {
    setState(() {
      _flashBgColor = flashColor.withOpacity(0.6);
    });
    Future.delayed(const Duration(milliseconds: 250), () {
      if (mounted) {
        setState(() {
          _flashBgColor = Colors.transparent;
        });
      }
    });
  }
  ...
  Widget _buildResetButton() {
    return InkWell(
      onTap: () async {
        _distFocus.unfocus();
        _triggerFlash(Colors.orange.shade700);
        final prefs = await SharedPreferences.getInstance();
        await prefs.reload();
      ...
      child: Padding(
        ...
        child: Icon(Icons.change_circle, color: Colors.orange.shade400.withOpacity(_opacity == 0.0 ? 0.0 : 1.0), size: 32),
      ),
    );
  }
  ...
  Widget _buildMainAppButton() {
    return InkWell(
      onTap: () async {
        _distFocus.unfocus();
        _triggerFlash(Colors.blue.shade700);
        final prefs = await SharedPreferences.getInstance();
        await prefs.setBool('overlay_is_returning', false);
        await _launchApp(_myPackage);
      },
  ...
  Widget _buildTargetAppButton() {
    ...
    return InkWell(
      onTap: () async {
        _distFocus.unfocus();
        _triggerFlash(Colors.green.shade700);
        final prefs = await SharedPreferences.getInstance();
      ...
      child: Padding(
        padding: const EdgeInsets.symmetric(horizontal: 10.0, vertical: 14.0), 
        child: Icon(Icons.layers, color: Colors.white.withOpacity(_opacity), size: 32),
      ),
    );
  }
  ...
  Widget _buildSecondTargetAppButton() {
    ...
    return InkWell(
      ...
      child: Padding(
        padding: const EdgeInsets.symmetric(horizontal: 10.0, vertical: 14.0), 
        child: Icon(Icons.rocket_launch, color: Colors.white.withOpacity(_opacity), size: 32),
      ),
    );
  }
                      ...
                      children: [
                        if (!_isShrunk) ...[
                              ...
                              child: Center(
                                child: IgnorePointer(
                                  child: TextField(
                                    ...
                                    style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Colors.black.withOpacity(_opacity == 0.0 ? 0.0 : 1.0)),
                          ...
                          Padding(
                            ...
                            child: Container(width: 75, height: 1, color: Colors.white.withOpacity(_opacity * 0.6)),
                          ), 
                        ],
                              ...
                              ? [
                                  InkWell(
                                    ...
                                    child: Padding(
                                      ...
                                      child: Icon(Icons.keyboard_hide, color: Colors.white.withOpacity(_opacity), size: 32),
                                    ),
                                  )
                                ]
                              : (_isShrunk
                                  ...
                                  : [
                                      ...
                                      if (hasTarget || hasSecondTarget)
                                        Row(
                                          mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                                          children: [
                                            if (hasTarget) _buildTargetAppButton(),
                                            if (hasSecondTarget) _buildSecondTargetAppButton(),
                                          ],
                                        ),
                                    ]),
                        ),
...
class _MockAppState extends State<MockApp> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    _overlaySubscription = FlutterOverlayWindow.overlayListener.listen((event) async {
      if (event == 'save_history') {
      ...
      } else if (event == 'ghost_mode_active') {
        if (mounted) _showMsg("Đã ẩn tàng hình nút nổi (Chống kill app)!");
        ...
        if (mounted) {
          ...
          _showMsg("Đã tắt hoàn toàn nút nổi!");
        }
        final prefs = await SharedPreferences.getInstance();
            ...
            if (nextIndex < beacons.length && beacons[nextIndex].location != null) {
                ...
                if (otherBeacons.length >= 3) {
                    ...
                    if (intersections.isNotEmpty) {
                        ...
                        newPos = _snapToAccurateBeacon(newPos, otherBeacons); 
                    }
          ...
          children: [
            ...
            ListTile(
              ...
              onTap: () async {
                ...
                _showMsg("Đã xóa liên kết App và tắt hoàn toàn nút nổi!");
              },
            ),
          ],
        ),
  ...
  LatLng _snapToAccurateBeacon(LatLng pt, List<Beacon> beacons) {
    Beacon? bestB;
    ...
    if (bestB != null && bestB.location != null) {
      if (minR == 0) return bestB.location!;
      double bearing = _calculateBearing(bestB.location!, pt);
      return _calculatePointFromBearing(bestB.location!, minR, bearing);
    }
    return pt;
  }
  ...
  LatLng? _internalCalculateBestFit() {
      ...
      for (int i = 0; i < validBeacons.length; i++) {
        for (int j = i + 1; j < validBeacons.length; j++) {
          ...
          var midInter = _calculateTwoCircleIntersectionPrecise(
              validBeacons[i].location!, r1Mid, 
              validBeacons[j].location!, r2Mid);
          candidates.addAll(midInter);
              
          if (midInter.isEmpty || midInter.length == 1) {
      ...
      LatLng optimizedPoint = _optimizePoint(bestPoint, validBeacons);
      return _snapToAccurateBeacon(optimizedPoint, validBeacons);
  }
  ...
  double _calculateTotalError(LatLng p, List<Beacon> beacons) {
    ...
    for (var b in beacons) {
      ...
      double maxErr = _getMaxError(b);
      double variance = maxErr * maxErr;
      if (variance < 1.0) variance = 1.0; 
      
      double circleSpread = (rMid <= 0) ? 1.0 : rMid; 
      
      double weight = 10000.0 / (variance * circleSpread);
  ...
  Future<void> _resetCurrentBeacon() async {
    ...
    if (otherBeacons.length >= 3) {
      ...
      if (beacons[targetIndex].location != null) {
        ...
        double newBearing = (r <= 30.0) ? currentBearing : (currentBearing + 60.0) % 360.0;
    ...
    if (newPos != null) {
      ...
      if (!isFixingNext) {
         if (otherBeacons.length >= 3) {
            ...
            _showMsg("Đã tạo lại hướng quét!");
         }
      } else {
      ...
      if (b1.location != null && b1.controller.text.isNotEmpty &&
          ...
          try {
            ...
            if (intersections.isNotEmpty) {
              ...
              finalPos = _snapToAccurateBeacon(finalPos, [b1, b2]);
              autoPlaced = true;
            }
          } catch (e) {}
      }
