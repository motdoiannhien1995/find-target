  ...
  Future<void> _launchApp(String pkg) async {
    try {
      final Uri uri = Uri.parse(
          "intent:#Intent;action=android.intent.action.MAIN;category=android.intent.category.LAUNCHER;launchFlags=0x10100000;package=$pkg;end");
      bool launched = await launchUrl(uri, mode: LaunchMode.externalNonBrowserApplication);
      if (launched) return;
    } catch (e) {
      print("Cách Intent lỗi: $e");
    }
            ...
            onTap: () async {
              ...
              if (_targetPackage != null) {
                if (_isReturning) {
                  ...
                  // Ra lệnh mở lại App Find Target
                  ...
                  // Tuyệt chiêu: Thay vì ép mở app mục tiêu (gây reset trang đang xem)
                  // Ta gửi lệnh yêu cầu App Find Target tự thu nhỏ xuống (Minimize).
                  // Lúc này, app mục tiêu nằm bên dưới sẽ tự động nổi lên nguyên trạng!
                  FlutterOverlayWindow.shareData('minimize_mockapp');
                }
              } else {
...
class _MockAppState extends State<MockApp> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    // Lắng nghe tín hiệu yêu cầu thu nhỏ App từ Cửa sổ nổi
    _overlaySubscription = FlutterOverlayWindow.overlayListener.listen((event) {
      if (!mounted) return;
      if (event == 'minimize_mockapp') {
        // Thu nhỏ App (Minimize), chuyển về chạy ngầm để lộ ra Target App y nguyên như cũ
  ...
  Future<void> _switchToTargetApp() async {
    ...
    // Thu nhỏ ứng dụng (Minimize) xuống bằng SystemNavigator.pop()
    // Nhờ thao tác này, app mục tiêu nằm bên dưới sẽ tự động được hiển thị 
    // giữ nguyên 100% màn hình tin nhắn cuối cùng mà không bị reset.
    SystemChannels.platform.invokeMethod('SystemNavigator.pop');
  }
                            ...
                            InkWell(
                              ...
                              onTap: () async {
                                if (secondTargetAppPackage == null) {
                                  ...
                                  try {
                                    final Uri uri = Uri.parse("intent:#Intent;action=android.intent.action.MAIN;category=android.intent.category.LAUNCHER;launchFlags=0x10100000;package=$secondTargetAppPackage;end");
                                    bool launched = await launchUrl(uri, mode: LaunchMode.externalNonBrowserApplication);
                                    if (!launched) {
