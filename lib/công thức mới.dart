...
class _MockAppState extends State<MockApp> with WidgetsBindingObserver {
  ...
  // --- [PHẦN SỬA ĐỔI QUAN TRỌNG: Đã xóa phần cộng dồn sai số 50m, 5m gây lỗi khoảng cách] ---
  double _getEffectiveRadius(Beacon b) {
    double r = _getRadiusInMeters(b);
    if (r <= 0) return 0;
    // Bỏ tất cả các số r + 50.0, r + 5.0 để hình học chuẩn xác nhất với input thực
    return r;
  }
  ...
  // --- [PHẦN SỬA ĐỔI QUAN TRỌNG: Cấu trúc lại bằng thuật toán Bình phương tối thiểu (Least Squares) để bù đắp việc làm tròn] ---
  double _calculateTotalError(LatLng p, List<Beacon> beacons) {
    ...
    for (var b in beacons) {
      ...
      // Thuật toán Bình phương tối thiểu: Tạo thung lũng dốc sâu để hội tụ chuẩn xác nhất.
      // Dù người dùng làm tròn thiếu (ví dụ vòng tròn không giao nhau), hệ thống vẫn tự trượt về điểm thỏa hiệp chính giữa.
      double weight = 1.0;
      if (r > 0) {
        weight = 1000.0 / (r + 100); // Trọng số mềm mại giúp bù trừ sai số khi nhập khoảng cách.
      }
