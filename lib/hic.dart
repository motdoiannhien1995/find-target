...
class _MockAppState extends State<MockApp> with WidgetsBindingObserver {
            ...
            if (nextIndex < beacons.length && beacons[nextIndex].location != null) {
                ...
                if (otherBeacons.length >= 3) {
                    ...
                    if (intersections.isNotEmpty) {
                        ...
                        // KHÔNG LÀM TRÒN NẾU CÓ MÉT
                        bool hasMeter = otherBeacons.any((b) => b.unit == 'm');
                        newPos = hasMeter ? chosen : _optimizePoint(chosen, otherBeacons);
                    }
  ...
  LatLng? _internalCalculateBestFit() {
      ...
      // BỎ LÀM TRÒN HOÀN TOÀN NẾU CÓ MỐC 'm'
      bool hasMeter = validBeacons.any((b) => b.unit == 'm');
      if (hasMeter) {
        return bestPoint;
      }
  ...
  double _calculateTotalError(LatLng p, List<Beacon> beacons) {
    ...
    for (var b in beacons) {
      ...
      double weight = 1.0;
      if (b.unit == 'm') {
        weight = 1000000.0; // Trọng số cực lớn, bắt buộc phải thỏa mãn khoảng cách 'm'
      } else {
        weight = 1.0; // Bù trừ bình thường với km
      }
    ...
    if (otherBeacons.length >= 3) {
      ...
      if (intersections.isNotEmpty) {
        if (intersections.length == 2 && beacons[targetIndex].location != null) {
          ...
          // KHÔNG LÀM TRÒN NẾU CÓ MÉT
          bool hasMeter = otherBeacons.any((b) => b.unit == 'm');
          newPos = hasMeter ? intersections[0] : _optimizePoint(intersections[0], otherBeacons);
        }
      }
    } else if (otherBeacons.length == 1) {
      var ref = otherBeacons[0];
      ...
      if (b1.location != null && b1.controller.text.isNotEmpty &&
          ...
          try {
            ...
            if (intersections.isNotEmpty) {
              ...
              // KHÔNG LÀM TRÒN NẾU CÓ MÉT
              bool hasMeter = [b1, b2].any((b) => b.unit == 'm');
              finalPos = hasMeter ? intersections[0] : _optimizePoint(intersections[0], [b1, b2]);
